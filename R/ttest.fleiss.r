#' @title Paired t-test for the difference of 2 Fleiss generalized kappa
#'        coefficients.
#'
#' @description This function performs the paired t-test for testing the
#'   difference between two correlated Fleiss kappa coefficients for
#'   statistical significance. It implements the linearization method
#'   of Gwet (2016).
#'
...
#'    \emph{Psychological Bulletin}, \strong{76}, 378-382.
#'
...
ttest.fleiss <- function(g1.ratings,g2.ratings,weights="unweighted",conflev=0.95,N=Inf){
  ...
  if (n2==n1){
    ...
    df.out <- data.frame(fleiss.coeff1, fleiss.coeff2,coeff.diff,std.err,t.stat,p.value,n.obs,n.raters1,n.raters2)
  }else{
